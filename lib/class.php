...
class Lentoria
{
    function __construct()
    {
        if (array_key_exists('addCategory', $_POST)) {
            ...
            $this->signupDetails();
        }  elseif (array_key_exists('activateCourse', $_POST)) {
            $this->activateCourse();
    ...
    function activateCourse(){
        global $db;
        $course_id = $_POST['activateCourse'];
        ...
        if(mysqli_num_rows($sql) == 0){
            ...
            while ($row = mysqli_fetch_array($sql)) {
                ...
                $sql2 = $db->query("SELECT * FROM course_topics WHERE module_id = '$module_id'");
        ...
        if($err> 0){
            global $count, $report, $db;
            $count = 1;
            $report = "This course cannot be activated because the modules do not contain topics";
        }else{

            
            global $count, $report, $db;
            $sql = $db->query("UPDATE courses SET status= 1 WHERE sn='$course_id' ") or die('cannot connect');

            $count = 0;
            $report = "You have successfully activated this course.";
